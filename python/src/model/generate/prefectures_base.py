...
class prefectures_base(timestamp_mixin, model):
    ...
    def get_prefecture_name_length(cls):
        return prefectures_base.__PREFECTURE_NAME_LENGTH
    ...
    def prefecture_name(cls):
        return model.get_db_instance(model).Column(VARBINARY(prefectures_base.__PREFECTURE_NAME_LENGTH), nullable = False, server_default = '', comment = '都道府県名')
    @declared_attr
