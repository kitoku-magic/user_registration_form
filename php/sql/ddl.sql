...
create table `user` (
  `user_id` varbinary(64) DEFAULT '' NOT NULL comment 'ユーザーID',
...
create table `tmp_user` (
  `tmp_user_id` varbinary(64) DEFAULT '' NOT NULL comment '一時ユーザーID',
...
) ENGINE=InnoDB comment='ユーザー一時情報';


drop table if exists `tmp_user_multiple_select`;

create table `tmp_user_multiple_select` (
  `tmp_user_id` varbinary(64) DEFAULT '' NOT NULL comment '一時ユーザーID',
  `multiple_select_id` mediumint unsigned DEFAULT 0 NOT NULL comment '複数選択項目ID',
  -- `is_selected` tinyint(1) DEFAULT 0 NOT NULL comment '選択済み状態',
  `created_at` bigint unsigned DEFAULT 0 NOT NULL comment '作成日時のタイムスタンプ',
  `updated_at` bigint unsigned DEFAULT 0 NOT NULL comment '更新日時のタイムスタンプ',
  PRIMARY KEY(`tmp_user_id`, `multiple_select_id`),
  CONSTRAINT fk_tmp_user_multiple_select_user_id FOREIGN KEY (tmp_user_id)
  REFERENCES `tmp_user` (tmp_user_id)
  ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT fk_tmp_user_multiple_select_multiple_select_id FOREIGN KEY (multiple_select_id)
  REFERENCES `multiple_select` (multiple_select_id)
  ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB comment='一時ユーザー複数選択項目';
